...
public class ImageUploadService {
    ...
    private byte[] compressBytes(byte[] data) {
        ...
        try {
            ...
            LOG.error("Не получается сжать байты");
        }
        System.out.println("Сжатие картинки размером - " + outputStream.toByteArray().length + " байт");
    ...
    private static byte[] decompressBytes(byte[] data) {
        ...
        try {
            ...
            LOG.error("Не вышло расжать байты");
        }
        return outputStream.toByteArray();
    }
    ...
    private <T> Collector<T, ?, T> toSinglePostCollector() {
        return Collectors.collectingAndThen(Collectors.toList(), new Function<List<T>, T>() {
            @Override
